...
static string_view get_dirname(const char* s) {
  ...
  return string_view(s, std::max(last_slash, last_backslash) - s);
}
...
static void print_call(const string_view& module_name,
                       const string_view& func_name,
...
struct Context {
  ...
  string_view source_filename;  // Memory owned by json_data.
  size_t json_offset;
  Location loc;
  Location prev_loc;
  bool has_prev_loc;
...
struct Action {
  ...
  string_view module_name;  // Memory owned by json_data.
  string_view field_name;   // Memory owned by json_data.
  std::vector<TypedValue> args;
};
...
static wabt::Result parse_string(Context* ctx, string_view* out_string) {
  *out_string = string_view();
  ...
  /* Modify json_data in-place so we can use the StringView directly
  ...
  *out_string = string_view(start, p - start);
  return wabt::Result::Ok;
}

static wabt::Result parse_key_string_value(Context* ctx,
                                           const char* key,
                                           string_view* out_string) {
  EXPECT_KEY(key);
...
static wabt::Result parse_const(Context* ctx, TypedValue* out_value) {
  ...
  if (type_str == "i32") {
    uint32_t value;
    CHECK_RESULT(parse_int32(value_str.begin(), value_str.end(), &value,
    ...
    uint32_t value_bits;
    CHECK_RESULT(parse_int32(value_str.begin(), value_str.end(), &value_bits,
                             ParseIntType::UnsignedOnly));
    ...
    uint64_t value;
    CHECK_RESULT(parse_int64(value_str.begin(), value_str.end(), &value,
                             ParseIntType::UnsignedOnly));
    ...
    uint64_t value_bits;
    CHECK_RESULT(parse_int64(value_str.begin(), value_str.end(), &value_bits,
                             ParseIntType::UnsignedOnly));
...
static std::string create_module_path(Context* ctx,
  ...
  std::string path;
  if (!dirname.empty()) {
    path = string_printf(PRIstringview, WABT_PRINTF_STRING_VIEW_ARG(filename));
  } else {
    path = string_printf(PRIstringview "/" PRIstringview,
                         WABT_PRINTF_STRING_VIEW_ARG(dirname),
                         WABT_PRINTF_STRING_VIEW_ARG(filename));
  }
...
static wabt::Result on_module_command(Context* ctx,
                                      const string_view& filename,
                                      const string_view& name) {
  std::string path = create_module_path(ctx, filename);
...
static wabt::Result on_assert_uninstantiable_command(
    Context* ctx,
    const string_view& filename,
    const string_view& text,
    ModuleType module_type) {
