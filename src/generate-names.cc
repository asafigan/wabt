...
class NameGenerator : public ExprVisitor::DelegateNop {
 ...
 private:
  static bool HasName(const string_view& str);
  static void GenerateName(const char* prefix, Index index, std::string* str);
...
// static
bool NameGenerator::HasName(const string_view& str) {
  return !str.empty();
}
...
void NameGenerator::GenerateName(const char* prefix,
  ...
  *str = string_view(buffer, actual_len).to_string();
}
