...
void write_str(Stream* stream,
  ...
  write_u32_leb128(stream, s.size(), "string length");
  stream->WriteData(s.begin(), s.size(), desc, print_chars);
}
...
void write_debug_name(Stream* stream,
                      ...
                      const char* desc) {
  string_view stripped(name);
  if (!stripped.empty()) {
    // Strip leading $ from name.
    assert(stripped[0] == '$');
    stripped.remove_prefix(1);
  }
  write_str(stream, stripped, desc, PrintChars::Yes);
}
